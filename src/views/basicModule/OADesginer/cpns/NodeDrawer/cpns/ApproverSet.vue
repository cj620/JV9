 ...
 * @LastEditTime: 2022-01-14 10:05:07
 * @Description: file content
-->
<template>
  <div class="approver-set">
    <!-- 审核类型选择 -->
    <el-radio-group v-model="nodeData.properties.auditWay"
                    @change="change">
      <el-row :gutter="20">
        <el-col :span="8"
    ...
    <div class="approver-action-wrapper">
      ...
      <el-button type="primary"
                 ...
                 @click="addStaffShow=true">{{ $t('setup.EditMembers') }}</el-button>
      <el-button type="primary"
                 icon="el-icon-plus"
                 round
                 v-show="typeSet.addRole">{{ $t('setup.EditRoles') }}</el-button>
      <div>
        <el-tag v-for="tag in choosedStaff"
                :key="tag.id"
                closable
        ...
        <el-link type="primary"
                 ...
                 v-show="choosedStaff.length!=0">{{ $t('Generality.Ge_Clear') }}</el-link>
      </div>
      <!-- 选择主管 -->
      <div v-show="typeSet.directorSelect">
        <span style="margin-right:5px">发起人的</span>
        <el-select v-model="testData"
                   size="mini">
          <el-option v-for="optionItem in typeSet.directorList"
    ...
    <!-- 审核方式模块 -->
    <div class="other-action-wrapper"
         v-show="auditTypeShow"
         v-for="auditItem in auditMap"
         :key="auditItem.title">
      <el-divider></el-divider>
      <div class="title">{{auditItem.title}}</div>
      <el-radio-group v-model="nodeData.properties.auditType"
                      @change="change">
        <el-row :gutter="20">
          <el-col :span="24"
            ...
            <el-radio :key="typeItem.value"
                      :label="typeItem.value">{{ typeItem.label }}{{typeItem.des}}</el-radio>
          </el-col>
        </el-row>
      </el-radio-group>

    </div>
    <StaffSelect :title="$t('Generality.Ge_DocumentName')"
                 :visible.sync="addStaffShow"
                 @complete="complete"
                 :defaultStaff="choosedStaff"></StaffSelect>
  </div>
</template>
