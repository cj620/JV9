...
import {
  API as stockInit
} from "@/api/workApi/stockroom/stockInit";
export default {
  name: "index",
  data() {
    return {
      ...
      btnAction: [],
      // 编辑路由
      editRouteName: "St_StockInit_Edit",
      // 打印模板标识 谨慎删除
  ...
  methods: {
    //获取数据
    async GetData() {
      await stockInit.api_get({ BillId: this.$route.query.BillId }).then((res) => {
        ...
        this.btnAction = detailPageModel(this, res, stockInit, this.GetData);
