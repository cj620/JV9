  ...
  <PageWrapper  ref="page">
    <div slot="sticky-tabs">
      <el-tabs @tab-click="tabClick">
        <!--单据信息-->
        <el-tab-pane :label="$t('Generality.Ge_BillInfo')" name="first"></el-tab-pane>
        <!--物料信息-->
        <el-tab-pane :label="$t('Generality.Ge_ItemsInfo')" name="second"></el-tab-pane>
        <!--备注-->
        <el-tab-pane :label="$t('Generality.Ge_Remarks')" name="third"></el-tab-pane>
        <!--附件-->
        <el-tab-pane :label="$t('Generality.Ge_Annex')" name="fourth"></el-tab-pane>
        <!--审核流程-->
        <el-tab-pane :label="$t('Generality.Ge_ApproveProcess')" name="fifth"></el-tab-pane>
      </el-tabs>
    </div>
...
import {
  API as returnPicking
} from "@/api/workApi/stockroom/returnPicking";
export default {
  name: "index",
  data() {
    return {
      ...
      btnAction: [],
      // 编辑路由
      editRouteName: "St_ReturnPicking_Edit",
      // 打印模板标识 谨慎删除
  ...
  methods: {
    //获取数据
    async GetData() {
      await returnPicking.api_get({ BillId: this.$route.query.BillId }).then((res) => {
        ...
        this.btnAction = detailPageModel(this, res, returnPicking, this.GetData);
      });
    },
    tabClick(e) {
