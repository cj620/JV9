...
import {
  API as produceIn
} from "@/api/workApi/stockroom/produceIn";
export default {
  name: "index",
  data() {
    return {
      ...
      btnAction: [],
      // 编辑路由
      editRouteName: "St_ProduceIn_Edit",
      // 打印模板标识 谨慎删除
  ...
  methods: {
    //获取数据
    async GetData() {
      await produceIn.api_get({ BillId: this.$route.query.BillId }).then((res) => {
        ...
        this.btnAction = detailPageModel(this, res, produceIn, this.GetData);
      });
    },
    tabClick(e) {
