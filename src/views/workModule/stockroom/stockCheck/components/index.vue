...
import { API as stockCheck } from "@/api/workApi/stockroom/stockCheck";
...
export default {
  ...
  methods: {
    //编辑的时候获取信息
    async GetData(Id) {
      await stockCheck.api_get({ BillId: Id }).then((res) => {
        this.ruleForm = res;
            ...
            this.eTableObj.validate((valid1) => {
              if (valid1) {
                stockCheck
                  ...
                  .then((res) => {
                    let TagName = {
                      path: "/picking/stockCheckDetails",
                      name: `St_StockCheck_Details`,
                      query: { BillId: res },
                      fullPath: "/picking/stockCheckDetails",
                    };
