...
import {
  API as stockCheck
} from "@/api/workApi/stockroom/stockCheck";
export default {
  name: "index",
  data() {
    return {
  ...
  methods: {
    //获取数据
    async GetData() {
      await stockCheck.api_get({ BillId: this.$route.query.BillId }).then((res) => {
        ...
        this.btnAction = detailPageModel(this, res, stockCheck, this.GetData);
      });
    },
