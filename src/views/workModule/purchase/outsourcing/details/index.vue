...
export default {
  ...
  data() {
    return {
      ...
      type: "",
      stateForm: {},
      editRouteName: "Pu_Outsourcing_Edit",
      addRouteName: "Pu_Outsourcing_Add",
      tabPanes: [
        {
  ...
  methods: {
    //获取数据
    GetData() {
      Outsourcing.api_get({ BillId: this.$route.query.BillId }).then((res) => {
        ...
        this.btnAction = detailPageModel(this, res, Outsourcing, this.GetData);
        this.btnAction.push({
    ...
    // 完成单据
    successOutsourcing() {
      successOutsourcing({ BillId: this.cur_billId }).then(() => {
        this.GetData();
      });
    },
  },
