    ...
    if (i === 0) {
      ...
      for (let j = startDay; j <= num; j++) {
        ...
        dayDetails.push({
          ...
          week: wk ? wk : 7, // 星期几
          number: 24,
          text: `${startYear}-${item < 10 ? "0" + item : item}-${
            j < 10 ? "0" + j : j
          }`,
        });
      }
      ...
      for (let j = 1; j <= endDay; j++) {
        ...
        dayDetails.push({
          ...
          week: wk ? wk : 7, // 星期几
          number: 24,
          text: `${endtYear}-${item < 10 ? "0" + item : item}-${
            j < 10 ? "0" + j : j
          }`,
        });
      }
      ...
      for (let j = 1; j <= days; j++) {
        ...
        dayDetails.push({
          ...
          week: wk ? wk : 7, // 星期几
          number: 24,
          text: `${monthDetails[i].year}-${item < 10 ? "0" + item : item}-${
            j < 10 ? "0" + j : j
          }`,
        });
      }
  ...
  dayDetails.forEach((item, i) => {
        ...
        weekDetails.push({
          ...
          text: `${item.year}-${
            item.month < 10 ? "0" + item.month : item.month
                    ...
                    ${dayDetails[i + j - 1].year}-${
            dayDetails[i + j - 1].month < 10
              ? "0" + dayDetails[i + j - 1].month
              : dayDetails[i + j - 1].month
          }-${
            dayDetails[i + j - 1].day < 10
              ? "0" + dayDetails[i + j - 1].day
              : dayDetails[i + j - 1].day
          }`,
        });
        // 时间开始的位置
      } else if (
    ...
    if (item.week === 7 && dayDetails[i - 6]) {
      weekDetails.push({
        ...
        endDay: item.day,
        number: 7,
        text: `${dayDetails[i - 6].year}-${
          dayDetails[i - 6].month < 10
            ? "0" + dayDetails[i - 6].month
            : dayDetails[i - 6].month
        }-${
          dayDetails[i - 6].day < 10
            ? "0" + dayDetails[i - 6].day
            : dayDetails[i - 6].day
        }  ~  
                ${item.year}-${
          item.month < 10 ? "0" + item.month : item.month
