...
class EditTable {
  ...
  validate = (callBack) => {
    ...
    let [valid, failMap, msg] = checkEditTable(
      this.row_required_props_map,
      this.tableData
    );
    //  执行回调
  ...
  borderFocus = (col, Rows, rowDoms) => {
    Rows.forEach((item) => {
      ...
      // 高亮当前的单元格
      rowChildrens[col + this.props.offSet].style =
        "box-shadow: inset 0 0 0 1px red;";
    });
  };
  // 失去焦点
  borderBlur = () => {
    let cellDoms = document
      .getElementById(this.id)
      .getElementsByClassName("jv-edit-table-cell");
